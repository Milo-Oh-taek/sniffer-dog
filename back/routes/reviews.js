  ...
  try {
    ...
    const reviewList = await Review.findAndCountAll({
      ...
      attributes: [
                    ...
                    [literal(`(SELECT COUNT(*) FROM user_review_like WHERE user_review_like.review_id = review.id)`), 'like_count'],
                    [literal(`(SELECT COUNT(*) FROM user_review_dislike WHERE user_review_dislike.review_id = review.id)`), 'dislike_count'],
                  ],
      include: [
        {
          model: Perfume,
